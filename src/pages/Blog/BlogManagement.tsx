      ...
      <div className="p-6 lg:p-8">
        <div className="flex flex-col gap-6 mb-8">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-3xl lg:text-4xl font-bold text-foreground mb-2">Manage Blogs</h1>
              <p className="text-muted-foreground">Create, edit, and manage your blog posts</p>
            </div>
            <Button onClick={() => navigate("/manage-blogs/create")} className="bg-primary hover:bg-primary/90">
        ...
        {loading ? (
          ...
          <div className="text-center py-12">
            ...
            <Button onClick={() => setStatusFilter("ALL")} variant="outline" className="border-border">
              Clear Filter
            </Button>
          </div>
        ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="border-border hover:bg-transparent">
                    <TableHead className="text-muted-foreground w-[100px] px-4">Image</TableHead>
                    <TableHead className="text-muted-foreground w-[250px] px-4">Title</TableHead>
                    <TableHead className="text-muted-foreground w-[180px] px-4">Author</TableHead>
                    <TableHead className="text-muted-foreground w-[180px] px-4">Date</TableHead>
                    <TableHead className="text-muted-foreground w-[120px] px-4">Status</TableHead>
                    <TableHead className="text-muted-foreground text-right w-[140px] px-4">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {blogs.map((blog) => (
                    <TableRow key={blog.id} className="border-border">
                      ...
                      <TableCell className="font-medium text-foreground px-4 py-4">
                        <span className="truncate block max-w-[250px]">{blog.title}</span>
                      </TableCell>
                      <TableCell className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-foreground truncate">{blog.authorName}</span>
                        </div>
                      </TableCell>
                      <TableCell className="px-4 py-4">
                      ...
                      <TableCell className="px-4 py-4">
                        <div className="flex items-center justify-end gap-1">
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => navigate(`/manage-blogs/edit/${blog.id}`)}
                            className="h-8 w-8 p-0 text-muted-foreground hover:text-foreground"
                          >
